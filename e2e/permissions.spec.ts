...
test.describe('Permissions', () => {
  ...
  test.describe('authenticated as super', () => {
    ...
    test('shows the permissions page with matrix', async ({ page }) => {
      ...
      await expect(page.getByRole('cell', { name: 'clients', exact: true })).toBeVisible()
      await expect(page.getByRole('cell', { name: 'permissions', exact: true })).toBeVisible()
    ...
    test('can delete a module', async ({ page }) => {
      ...
      const deleteBtn = page.getByRole('row').filter({ hasText: 'todelete' }).getByRole('button').last()
      await deleteBtn.click()
      await expect(page.getByText('todelete')).not.toBeVisible({ timeout: 10000 })
    })

    test('shows toggle switches for each permission-role cell', async ({ page }) => {
  ...
  test.describe('authenticated as admin', () => {
    ...
    test('does not show Permissions link in sidebar', async ({ page }) => {
