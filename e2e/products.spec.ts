...
test.describe("Products", () => {
  ...
  test.describe("authenticated as super", () => {
    ...
    test("opens create dialog on Add Product click", async ({
      page,
    }) => {
      await page.getByRole("button", { name: /Add Product/i }).click()
      ...
      await expect(
        dialog.getByRole("heading", { name: "Add Product" })
      ).toBeVisible()
    })
    ...
    test("super can see delete button in actions menu", async ({
      page,
    }) => {
    ...
    test("deletes a product from actions menu", async ({ page }) => {
      ...
      await expect(
        deleteDialog.getByRole("heading", { name: "Delete" })
      ).toBeVisible()
