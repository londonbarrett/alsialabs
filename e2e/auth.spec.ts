...
async function blockOAuthRedirects(page: Page) {
  ...
  await page.route("**/www.facebook.com/**", (route) =>
    route.fulfill({ status: 200, body: "<html></html>" })
  )
}
...
test.describe("Auth", () => {
  ...
  test("displays sign-in buttons for Google and Facebook", async ({
    page,
  }) => {
    await expect(
    ...
    await expect(
      page.getByRole("button", { name: /Sign in with Facebook/i })
    ).toBeVisible()
  })

  test("shows a heading and description on the login page", async ({
    page,
  }) => {
    await expect(
    ...
    await expect(
      page.getByText(/Choose your authentication provider/i)
    ).toBeVisible()
  })
  ...
  test("shows connecting state while OAuth redirect is in progress", async ({
    page,
  }) => {
