...
- **WHEN** a component calls `useLoadingIndicator().start()` or `useServerAction` executes a server action
...
The system SHALL provide a `useLoadingIndicator` hook that returns `{ isLoading, start, stop }`. Any client component MAY call `start()` to show the bar and `stop()` to hide it.
