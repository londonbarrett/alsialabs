...
export async function getProjectCategories() {
  const t = await getActionT('actions.project-categories')
  try {
    await requirePermission('project-categories', 'view')
  } catch {
...
export async function upsertProjectCategory(data: ProjectCategoryFormData, id?: string) {
  const t = await getActionT('actions.project-categories')
  try {
    await requirePermission('project-categories', id ? 'edit' : 'create')
  } catch {
...
export async function deleteProjectCategory(id: string) {
  const t = await getActionT('actions.project-categories')
  try {
    await requirePermission('project-categories', 'delete')
  } catch {
    return { success: false as const, error: t('forbidden') }
  }
